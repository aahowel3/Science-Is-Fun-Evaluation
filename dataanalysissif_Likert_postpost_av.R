...
data=read.csv("df_av_melt_f.csv",header=TRUE)

#checking Chronbach's alpha reliability 
data_a=read.csv("av.csv",header=TRUE)
r=alpha(data_a)
r
...
datapre= subset(data,data$condition=="Pre AV")
datapost= subset(data,data$condition=="Post AV")
datapostpost= subset(data,data$condition=="PostPost AV")
...
data$factor=factor(data$condition,levels=c("Pre AV","Post AV","PostPost AV"))
boxplot(score ~ factor, data=data, main="Additudes and Values Scores Per Assessment Period",xlab="",ylab="",ylim=c(1,5),outline=FALSE)
#text(1:3, labels=c("a","b","c"),pos=3)
text(tapply(data$score,data$condition) - 1.95, labels=c("a","a","a"),pos=3)
...
plotCI(1:3, c(medianpre_post[[9]],medianpost_postpost[[9]],medianpre_postpost[[9]]), 
       ...
       xlab="", ylab="Median Difference Additudes and Values Scores", 
       main = "95% CI Median Difference Additudes and Values Scores")
axis(side=1, at=1:3,label=c("95% CI Pre-Post Additudes and Values","95% CI Post-PostPost Additudes and Values",
                            "95% CI Pre-PostPost Additudes and Values"))
...
plotCI(1:3, c(mean_pre,mean_post,mean_postpost), uiw = c(2*std_dev_pre,2*std_dev_post,2*std_dev_postpost), lty = 2, xaxt ="n", 
       gap = 0,ylim=c(1,5),xlab="", ylab="Mean Additudes and Values Scores", 
       main = "95% CI Mean Additudes and Values Scores")

axis(side=1, at=1:3,label=c("95% CI Pre Additudes and Values","95% CI Post Additudes and Values",
                            "95% CI PostPost Additudes and Values"))
...
data1=read.csv("df_av_melt_t.csv",header=TRUE)
datapre= subset(data1,data1$condition=="Pre AV")
datapost= subset(data1,data1$condition=="Post AV")
...
data1$factor=factor(data1$condition,levels=c("Pre AV","Post AV"))
boxplot(score ~ factor, data=data1, main="Additudes and Values Scores Per Assessment Period",xlab="",ylab="",ylim=c(1,5),outline=FALSE)
#text(1:3, labels=c("a","b","c"),pos=3)
...
plotCI(1:1, medianpre_post[[9]], 
       ...
       xlab="", ylab="Median Difference Additudes and Values Scores", 
       main = "95% CI Median Difference Additudes and Values Scores")
axis(side=1, at=1:1,label="95% CI Pre-Post Additudes and Values")

#mean of each group with 2SD CIs 
#more representative of data, easily interperted 
library("gplots")
plotCI(1:2, c(mean_pre,mean_post), uiw = c(2*std_dev_pre,2*std_dev_post), lty = 2, xaxt ="n", 
       gap = 0,xlim=c(0.8,2),ylim=c(1,5),xlab="", ylab="Mean Additudes and Values Scores", 
       main = "95% CI Mean Additudes and Values Scores")

axis(side=1, at=1:2,label=c("95% CI Pre Additudes and Values","95% CI Post Additudes and Values"))
