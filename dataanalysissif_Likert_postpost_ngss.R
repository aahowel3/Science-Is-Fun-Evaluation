...
data=read.csv("df_ngss_melt_f.csv",header=TRUE)

#checking Chronbach's alpha reliability 
data_a=read.csv("ngss.csv",header=TRUE)
r=alpha(data_a)
r
...
datapre= subset(data,data$condition=="Pre NGSS")
datapost= subset(data,data$condition=="Post NGSS")
datapostpost= subset(data,data$condition=="PostPost NGSS")
...
data$factor=factor(data$condition,levels=c("Pre NGSS","Post NGSS","PostPost NGSS"))
boxplot(score ~ factor, data=data, main="Self-Efficacy Scores Per Assessment Period",xlab="",ylab="",ylim=c(1,5),outline=FALSE)
...
plotCI(1:3, c(medianpre_post[[9]],medianpost_postpost[[9]],medianpre_postpost[[9]]), 
       ...
       xlab="", ylab="Median Difference Self-Efficacy Scores", 
       main = "95% CI Median Difference Self-Efficacy Scores")
axis(side=1, at=1:3,label=c("95% CI Pre-Post Self-Efficacy","95% CI Post-PostPost Self-Efficacy",
                            "95% CI Pre-PostPost Self-Efficacy"))
...
plotCI(1:3, c(mean_pre,mean_post,mean_postpost), uiw = c(2*std_dev_pre,2*std_dev_post,2*std_dev_postpost), lty = 2, xaxt ="n", 
       gap = 0,ylim=c(1,5),xlab="", ylab="Mean Self-Efficacy Scores", 
       main = "95% CI Mean Self-Efficacy Scores")

axis(side=1, at=1:3,label=c("95% CI Pre Self-Efficacy","95% CI Post Self-Efficacy","95% CI PostPost Self-Efficacy"))
...
data1=read.csv("df_ngss_melt_t.csv",header=TRUE)
datapre= subset(data1,data1$condition=="Pre NGSS")
datapost= subset(data1,data1$condition=="Post NGSS")

data1$factor=factor(data1$condition,levels=c("Pre NGSS","Post NGSS"))
boxplot(score ~ factor, data=data1, col=(c("darkgreen","gold")),
        main="Self-Efficacy Scores Per Assessment Period",xlab="",ylab="",ylim=c(1,5),outline=FALSE,
        whisklty=0,staplelty=0)
#text(1:3, labels=c("a","b","c"),pos=3)
text(tapply(data$score,data$condition) - 1.8, labels=c("a","b"),pos=3)
...
data1$factor=factor(data1$condition,levels=c("Pre NGSS","Post NGSS"))
boxplot(score ~ factor, data=data1, main="Self-Efficacy Scores Per Assessment Period",xlab="",ylab="",ylim=c(1,5),outline=FALSE)
#text(1:3, labels=c("a","b","c"),pos=3)
...
plotCI(1:1, medianpre_post[[9]], 
       ...
       xlab="", ylab="Median Difference Self-Efficacy Scores", 
       main = "95% CI Median Difference Self-Efficacy Scores")
axis(side=1, at=1:1,label="95% CI Pre-Post Self-Efficacy")

#mean of each group with 2SD CIs 
#more representative of data, easily interperted 
library("gplots")
plotCI(1:2, c(mean_pre,mean_post), uiw = c(2*std_dev_pre,2*std_dev_post), lty = 2, xaxt ="n", 
       gap = 0,xlim=c(0.8,2),ylim=c(1,5),xlab="", ylab="Mean Self-Efficacy Scores", 
       main = "95% CI Mean Self-Efficacy Scores")

axis(side=1, at=1:2,label=c("95% CI Pre Self-Efficacy","95% CI Post Self-Efficacy"))
