...
data=read.csv("df_e_melt_f.csv",header=TRUE)

#checking Chronbach's alpha reliability 
data_a=read.csv("expectancy.csv",header=TRUE)
r=alpha(data_a)
r
...
datapre= subset(data,data$condition=="Pre E")
datapost= subset(data,data$condition=="Post E")
datapostpost= subset(data,data$condition=="PostPost E")
...
data$factor=factor(data$condition,levels=c("Pre E","Post E","PostPost E"))
boxplot(score ~ factor, data=data, main="Expectancy Scores Per Assessment Period",xlab="",ylab="",ylim=c(1,5),outline=FALSE)
...
plotCI(1:3, c(medianpre_post[[9]],medianpost_postpost[[9]],medianpre_postpost[[9]]), 
       ...
       xlab="", ylab="Median Difference Expectancy Scores", 
       main = "95% CI Median Difference Expectancy Scores")
axis(side=1, at=1:3,label=c("95% CI Pre-Post Expectancy","95% CI Post-PostPost Expectancy","95% CI Pre-PostPost Expectancy"))
...
plotCI(1:3, c(mean_pre,mean_post,mean_postpost), uiw = c(2*std_dev_pre,2*std_dev_post,2*std_dev_postpost), lty = 2, xaxt ="n", 
       gap = 0,ylim=c(1,5),xlab="", ylab="Mean Expectancy Scores", 
       main = "95% CI Mean Expectancy Scores")

axis(side=1, at=1:3,label=c("95% CI Pre Expectancy","95% CI Post Expectancy","95% CI PostPost Expectancy"))
...
data1=read.csv("df_e_melt_t.csv",header=TRUE)
datapre= subset(data1,data1$condition=="Pre E")
datapost= subset(data1,data1$condition=="Post E")
...
data1$factor=factor(data1$condition,levels=c("Pre E","Post E"))
boxplot(score ~ factor, data=data1, main="Expectancy Scores Per Assessment Period",xlab="",ylab="",ylim=c(1,5),outline=FALSE)
#text(1:3, labels=c("a","b","c"),pos=3)
...
plotCI(1:1, medianpre_post[[9]], 
       ...
       xlab="", ylab="Median Difference Expectancy Scores", 
       main = "95% CI Median Difference Expectancy Scores")
axis(side=1, at=1:1,label="95% CI Pre-Post Expectancy")

#mean of each group with 2SD CIs 
#more representative of data, easily interperted 
library("gplots")
plotCI(1:2, c(mean_pre,mean_post), uiw = c(2*std_dev_pre,2*std_dev_post), lty = 2, xaxt ="n", 
       gap = 0,xlim=c(0.8,2),ylim=c(1,5),xlab="", ylab="Mean Expectancy Scores", 
       main = "95% CI Mean Expectancy Scores")

axis(side=1, at=1:2,label=c("95% CI Pre Expectancy","95% CI Post Expectancy"))
